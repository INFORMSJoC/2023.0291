...
# library variables
sfExport('settings','k.all','R.r','n.s','sum')
...
main_parallel <-function(ii){
  ...
  FPE<-1/sqrt(2*pi)*exp(-q^2/2)*10.01 # the item to be subtracted
  k<-M<-floor(5*n^(1/5))
  # get theta from R.square
  theta<- sqrt((120.201/(1-R.square)-120.201)/(sum/12)) 
  # construct the coefficients
  beta<-matrix(0,k.all,1)
  for (j in 1:k.all) {
    beta[j]<-theta*j^(-1)
  }
  ...
  for (r in 1:R.r) {
    ...
    eplison<-matrix(0.01,n,1)
    eplison.s<-matrix(0.01,n.s,1)
    for (j in 2:11) {
    ...
    y<-X[,1]*beta[1]+pnorm(X[,2:k.all]) %*% beta[2:k.all]+eplison
    y.s<-X.s[,1]*beta[1]+pnorm(X.s[,2:k.all]) %*% beta[2:k.all]+eplison.s
    X<-X[,1:k]
    X.s<-X.s[,1:k]
    data<-data.frame(y,X)
...
main_parallel <-function(ii){
  ...
  FPE<-((1-2*tau)*e*1/sqrt(2*pi)*exp(-e^2/2)+(1-2*tau)*pnorm(e)+
          tau+(1-2*tau)*e^2*pnorm(e)+tau*e^2)*120.201 
  k<-M<-floor(5*n^(1/5))
  # get theta from R.square
  theta<- sqrt((120.201/(1-R.square)-120.201)/(sum/12)) 
  # construct the coefficients
  ...
  EFPE<-min(colMeans(EFPE0))
  EFPE.JCVMA5<-mean(EFPEr.JCVMA5,na.rm = TRUE,trim=0.05)/EFPE
  EFPE.JCVMA10<-mean(EFPEr.JCVMA10,na.rm = TRUE,trim=0.05)/EFPE
  EFPE.JMA<-mean(EFPEr.JMA,na.rm = TRUE,trim=0.05)/EFPE
  EFPE.SAIC<-mean(EFPEr.SAIC,trim=0.05)/EFPE
  EFPE.SBIC<-mean(EFPEr.SBIC,trim=0.05)/EFPE
  EFPE.AIC<-mean(EFPEr.AIC,trim=0.05)/EFPE
  EFPE.BIC<-mean(EFPEr.BIC,trim=0.05)/EFPE
  EFPE.MAP5<-mean(EFPEr.MAP5,trim=0.05)/EFPE
  EFPE.MAP10<-mean(EFPEr.MAP10,trim=0.05)/EFPE
  EFPE.EWA<-mean(EFPEr.EWA,trim=0.05)/EFPE
  return(c(n,M,tau,R.square,EFPE.JCVMA5,EFPE.JCVMA10,EFPE.JMA,
           EFPE.SAIC,EFPE.SBIC,EFPE.MAP5,EFPE.MAP10,
           EFPE.EWA,EFPE.AIC,EFPE.BIC))
}
...
for (tau in unique(EFPE$tau)) {
  pdf(paste('DGP3_QR_',tau,'.pdf',sep=""),width = 10,height = 8)
  ...
  for (n in unique(EFPE$n)) {
    ...
    plot(x = EFPEn$R2,y = EFPEn$JCVMA5,
         ...
         lwd=2.5,type='o',lty=1,ylim = c(0.7,1.5),pch=4, xpd=FALSE)
...
###################### p=2 ##########################
for (tau in unique(EFPE$tau)) {
  pdf(paste('DGP3_ER_',tau,'.pdf',sep=""),width = 10,height = 8)
  par(mfrow = c(2, 2),oma = c(3.2, 0.5, 0.1, 0.6),xpd = NA,
      cex.main = 1.8, cex.lab = 1.5, cex.axis = 1.4) 
  for (n in unique(EFPE$n)) {
    EFPEn<-EFPE[EFPE$n==n & EFPE$tau==tau,]
