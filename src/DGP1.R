...
main_parallel <-function(ii){
  ...
  FPE<-1/sqrt(2*pi)*exp(-q^2/2)*5 # the item to be subtracted
  k<-M<-floor(5*n^(1/5))
  # get theta from R.square
  theta<-sqrt((35/(1-R.square)-35)/sum)
...
main_parallel <-function(ii){
  ...
  FPE<-((1-2*tau)*e*1/sqrt(2*pi)*exp(-e^2/2)+(1-2*tau)*pnorm(e)+
          tau+(1-2*tau)*e^2*pnorm(e)+tau*e^2)*35 
  k<-M<-floor(5*n^(1/5))
  # get theta from R.square
  theta<-sqrt((35/(1-R.square)-35)/sum)     
  # construct the coefficients
  ...
  EFPE<-min(colMeans(EFPE0))
  EFPE.JCVMA5<-mean(EFPEr.JCVMA5,na.rm = TRUE)/EFPE
  EFPE.JCVMA10<-mean(EFPEr.JCVMA10,na.rm = TRUE)/EFPE
  EFPE.JMA<-mean(EFPEr.JMA,na.rm = TRUE)/EFPE
  EFPE.SAIC<-mean(EFPEr.SAIC)/EFPE
  EFPE.SBIC<-mean(EFPEr.SBIC)/EFPE
  EFPE.AIC<-mean(EFPEr.AIC)/EFPE
  EFPE.BIC<-mean(EFPEr.BIC)/EFPE
...
for (tau in unique(EFPE$tau)) {
  pdf(paste('DGP1_QR_',tau,'.pdf',sep=""),width = 10,height = 8)
...
###################### p=2 ##########################
for (tau in unique(EFPE$tau)) {
  pdf(paste('DGP1_ER_',tau,'.pdf',sep=""),width = 10,height = 8)
  par(mfrow = c(2, 2),oma = c(3.2, 0.5, 0.1, 0.6),xpd = NA,
      cex.main = 1.8, cex.lab = 1.5, cex.axis = 1.4)  
  for (n in unique(EFPE$n)) {
    EFPEn<-EFPE[EFPE$n==n & EFPE$tau==tau,]
